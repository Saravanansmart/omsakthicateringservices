# Om Shakti Catering — Offer Page Setup

This covers the **`/offers/`** scratch-card promo page **(OTP step removed)**. The
guest enters their mobile number → the number is saved to your Google Sheet with a
**unique, non-repeating coupon code** → they land straight on the scratch-card
offer reveal.

## Files
- `offers/index.html` — the offer page (self-contained; open it directly or host it).
- `assets/catering-hero.jpg` — the hero image (shared with the homepage).
- `google-apps-script.gs` — the unified backend: writes offer-page submissions
  (this doc) *and* homepage enquiry-form submissions to the same sheet, in
  separate tabs, routed by a `type` field.
...
5. Open `offers/index.html`, find this line near the bottom, and paste your URL:
   ```js
   const SCRIPT_URL = "PASTE_YOUR_APPS_SCRIPT_WEB_APP_URL_HERE";
   ```
   (The homepage's enquiry form uses the same URL, as `LEAD_SCRIPT_URL` in `js/main.js`.)
...
```
Open http://localhost:8777 for the homepage, or http://localhost:8777/offers/ for
the offer page. Until `SCRIPT_URL` is set, the offer page runs in **preview
mode**: the full flow works but nothing is saved to the sheet (a console note
confirms this).

## Deploy the site
Host the whole repo on any static host (Netlify, Vercel, GitHub Pages, Firebase
Hosting, or your own web server). No build step — see `README.md`.
